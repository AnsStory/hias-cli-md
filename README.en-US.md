...
| --- | --- | --- |
| `sourceLocale` | `""` | Language currently used in source files; empty means `locales[0]` |
| `locales` | `["zh-CN", "en-US"]` | Locale files to generate; target locales exclude the source locale |
...
When `sourceLocale` is empty, `locales[0]` is treated as the source locale for backward compatibility.
